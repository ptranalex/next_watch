...
# SQLAlchemy and Redis instrumentors are imported conditionally when needed
from opentelemetry.propagate import set_global_textmap
from opentelemetry.propagators.b3 import B3MultiFormat
from opentelemetry.propagators.composite import CompositePropagator
from opentelemetry.propagators.jaeger import JaegerPropagator

# TraceContextPropagator not available in this OpenTelemetry version
...
# Note: RequestIDTracingMiddleware has been superseded by RequestContextMiddleware
# which provides more comprehensive request context management, trace propagation,
# user ID extraction, and automatic header injection for downstream services.
# See fast_core.middleware.context for the new implementation.
...
def setup_tracing(app: FastAPI, settings: Any) -> None:
    ...
    """
    try:
        # Get tracing configuration - check if settings itself has tracing config
        # or if it has a separate monitoring object
        if hasattr(settings, "enable_tracing"):
            # Settings object itself has tracing configuration (e.g., FastAPIConfig with MonitoringConfigMixin)
            monitoring = settings
        else:
            # Look for separate monitoring configuration object
        ...
        if tracing_endpoint:
            # Production: Export to Tempo via OTLP
        ...
        # Set up propagators for cross-service tracing with W3C Trace Context as primary
        try:
            ...
            # Fallback if W3C propagator not available
            propagators = [
                B3MultiFormat(),
                JaegerPropagator(),
            ]
        ...
        FastAPIInstrumentor.instrument_app(
            ...
            meter_provider=None,  # We handle metrics separately
        )
        ...
        # Instrument database connections if available
        try:
        ...
        # Note: Request ID and context management is now handled by RequestContextMiddleware
        # which provides more comprehensive tracing context, user ID extraction, and
        # automatic trace header propagation. No need for separate RequestIDTracingMiddleware.
        ...
        try:
            ...
            if context:
                ...
                if context.service_name:
                    record.service_name = context.service_name  # Override with context service name
        except ImportError:
...
def add_span_attributes(**attributes: Any) -> None:
    ...
    try:
        ...
        if span.is_recording():
            ...
            # Auto-add request context if available
            try:
                # Import here to avoid circular imports
                from fast_core.middleware.context import get_request_context
