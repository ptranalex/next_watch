...
class RequestContext:
    ...
    def get_propagation_headers(self) -> dict[str, str]:
        ...
        # Include original trace headers for propagation
...
class RequestContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        # Extract trace headers from incoming request (if trace propagation enabled)
        trace_headers = {}
        if self.trace_propagation:
            trace_headers = self._extract_trace_headers(request)

        # Set up OpenTelemetry context from headers (if trace propagation enabled)
        trace_id, span_id, parent_span_id = None, None, None
        if self.trace_propagation:
            carrier = dict(request.headers)
            ctx = propagate.extract(carrier)

            # Get trace information from OpenTelemetry context
            trace_id, span_id, parent_span_id = self._get_trace_info(ctx)

        # Extract user ID if available (if extraction enabled)
        user_id = None
        ...
        # Set context variables for the request scope
        ...
        # Store in request state for FastAPI compatibility
        request.state.request_id = request_id
        request.state.request_context = context

        # Add request context attributes to current OpenTelemetry span
        if self.trace_propagation:
        ...
        try:
            # Process request with OpenTelemetry context
            with trace.use_span(
    ...
    def _extract_request_id(self, request: Request) -> str:
        ...
        Returns:
            Request ID string
        """
        # Try multiple headers for request ID
        for header in ["X-Request-ID", "X-Correlation-ID", "X-Trace-ID"]:
            request_id = request.headers.get(header)
            if request_id:
                return request_id

        # Generate new UUID if no request ID found and auto-generation is enabled
        if self.auto_generate_request_id:
            return str(uuid.uuid4())

        # If auto-generation is disabled, return a default
    ...
    def _extract_trace_headers(self, request: Request) -> dict[str, str]:
        ...
        # W3C Trace Context headers (primary standard)
        ...
        # Custom request correlation headers (always include for request ID tracking)
    ...
    def _get_trace_info(self, ctx: Any | None = None) -> tuple[str | None, str | None, str | None]:
        ...
        try:
            ...
            if span and span.is_recording():
                ...
                # Get parent span ID if this is a child span
                parent_span_id = None
    ...
    def _add_span_attributes(self, context: RequestContext) -> None:
        ...
        try:
            ...
            if span and span.is_recording():
                # Add request ID attributes (both formats for compatibility)
                span.set_attribute("request.id", context.request_id)
                span.set_attribute("http.request_id", context.request_id)  # Legacy compatibility

                # Add user and service context
                if context.user_id:
    ...
    def _add_span_event(self, context: RequestContext, request: Request) -> None:
        ...
        try:
            span = trace.get_current_span()
            if span and span.is_recording():
                # Build event attributes, only including non-None values
                event_attributes = {
                ...
                # Add span event for better searchability
