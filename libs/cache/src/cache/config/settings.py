...
class CacheSettings(BaseSettings):
    ...
    ***REMOVED*** Cache behavior settings
    key_prefix: str = Field(default="nextwatch", description="Global key prefix for all cache keys")
    ...
    ***REMOVED*** Domain-specific TTL settings (in seconds)
    ttl_movie_data: int = Field(default=600, description="TTL for movie data cache")  ***REMOVED*** 10 minutes
    ttl_user_session: int = Field(default=3600, description="TTL for user session cache")  ***REMOVED*** 1 hour
    ttl_popular_content: int = Field(
        default=1800, description="TTL for popular content cache"  ***REMOVED*** 30 minutes
    )
    ttl_default: int = Field(default=300, description="Default TTL when not specified")  ***REMOVED*** 5 minutes
    ...
    def get_ttl_for_domain(self, domain: str) -> int:
        ...
        return domain_ttl_map.get(domain, self.ttl_default)
