...
const logger = createLogger("useLikedPage");

// Type for liked movies response
...
/**
 * Hook for managing liked movies page data with filtering and pagination
 ...
 */
export function useLikedPage() {
  const queryClient = useQueryClient();

  // Log hook initialization
  logger.debug(`useLikedPage initialized`);
  ...
  const queryKey = useMemo(() => {
    const baseKey = CacheKeys.movies.user.liked();
  ...
  // Fetch liked movies with pagination support and filtering
  const {
    data: likedData,
    isLoading,
    isFetchingNextPage,
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const baseUrl = `/bff/v1/liked?page=${pageParam}&limit=20`;
      const urlWithFilters = queryString
        ? `${baseUrl}&${queryString}`
        : baseUrl;

      logger.info(
        `Fetching liked movies page ${pageParam} with filters: ${queryString}`
      );

      const response = await fetchData<LikedMoviesResponse>(urlWithFilters);
    ...
    onSuccess: (data: InfiniteData<LikedMoviesResponse>) => {
      // Log successful data loading
      const firstPage = data?.pages?.[0];
      if (firstPage) {
        logger.info("Loaded liked movies successfully", {
        ...
        if (firstMovie) {
          logger.debug("Sample liked movie with user interactions:", {
            id: firstMovie.id,
    ...
    onError: (error: unknown) => {
      ...
      if (apiError.status === 404) {
        logger.info("Liked movies not found (404)");
      } else {
        logger.error("Error loading liked movies:", error);
      }
    },
  });
  ...
  const allMovies = useMemo(() => {
    ...
    const movies = likedData.pages.flatMap(
      (page) => page.results || []
    ) as Movie[];

    logger.debug(
      `Total liked movies loaded: ${movies.length} (with filters: ${queryString})`
    );
    return movies;
  }, [likedData?.pages, queryString]);
  ...
  const latestPage = likedData?.pages?.[likedData.pages.length - 1];
  ...
  const loadMore = () => {
    if (hasNextPage && !isFetchingNextPage) {
      logger.info(
        `Loading more liked movies, page ${
          currentPage + 1
  ...
  useEffect(() => {
    if (error) {
      logger.error(`Error fetching liked movies:`, error);
    }
  }, [error]);

  // Log filter changes
  useEffect(() => {
    logger.info(`Liked movies filters updated:`, queryParams);
  }, [queryParams]);
  ...
  useEffect(() => {
    if (error) {
      ...
      logger.info(
        `Fetched ${fetchedMoviesCount} liked movies from ${
          likedData?.pages?.length || 0
        } pages (total: ${totalMovies})`
      );

      // Log a sample movie to verify user interaction data
      const firstMovie = likedData?.pages?.[0]?.results?.[0];
      if (firstMovie) {
        logger.debug("Sample liked movie with user interactions:", {
          id: firstMovie.id,
          title: firstMovie.title,
          ...
          liked: firstMovie.liked,
          in_watchlist: firstMovie.in_watchlist,
          user_interactions: firstMovie.user_interactions,
        });
      }
    }
  }, [error, fetchedMoviesCount, totalMovies, likedData?.pages]);
