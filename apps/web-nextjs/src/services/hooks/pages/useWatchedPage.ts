...
export function useWatchedPage() {
  ...
  const {
    ...
    onSuccess: (data: InfiniteData<WatchedMoviesResponse>) => {
      // Log successful data loading
      const firstPage = data?.pages?.[0];
      if (firstPage) {
        logger.info("Loaded watched movies successfully", {
    ...
    onError: (error: unknown) => {
      ...
      if (apiError.status === 404) {
        logger.info("Watched movies not found (404)");
      } else {
        logger.error("Error loading watched movies:", error);
      }
    },
  });
