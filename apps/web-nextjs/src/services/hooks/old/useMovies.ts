...
// Types for different response formats
interface StandardizedPaginationResponse {
  total: number;
  page: number;
  per_page: number;
  total_pages: number;
...
// Helper function to determine pagination info
function getPaginationInfo(response: ApiResponse): {
  hasResults: boolean;
  currentPage: number;
  hasNext: boolean;
  movies: Movie[];
  ...
  if ("results" in response) {
    ...
    return {
      hasResults: response.movies.length > 0,
      currentPage: response.page,
      hasNext: response.movies.length === 20, // Assume full page means more available
...
export const useMovies = (options: UseMoviesOptions) => {
  ...
  const {
    ...
    getNextPageParam: (lastPage) => {
      ...
      const paginationInfo = getPaginationInfo(lastPage as ApiResponse);

      if (!paginationInfo.hasResults) return undefined;

      // Don't attempt to load more pages for watchlist and favorites
      if (options.source === "watchlist" || options.source === "favorites")
        return undefined;

      logger.debug(`Next page determined: ${paginationInfo.currentPage + 1}`);
  ...
  useEffect(() => {
    if (data) {
      const totalMovies = data.pages.reduce((sum, page) => {
        const paginationInfo = getPaginationInfo(page as ApiResponse);
        return sum + paginationInfo.movies.length;
      }, 0);
      logger.info(
  ...
  const { mutate: updateMovie } = useMutation({
    ...
    onSuccess: (updatedMovie: Movie) => {
        ...
        (oldData) => {
          ...
          const newData = {
            ...
            pages: oldData.pages.map((page) => {
              const paginationInfo = getPaginationInfo(page);
              const movieIndex = paginationInfo.movies.findIndex(
              ...
              if (movieIndex >= 0) {
                ...
                const updatedMovies = [...paginationInfo.movies];
                updatedMovies[movieIndex] = {
                  ...updatedMovies[movieIndex],
                  ...updatedMovie,
                };

                // Return updated page in the correct format
                if ("results" in page) {
                  return {
                    ...page,
      ...
      queryClient.setQueryData(
        ["movie", updatedMovie.id],
        (oldData: Movie | undefined) => {
          if (!oldData) return oldData;

          logger.debug(
            `Updating movie ${updatedMovie.id} in single movie cache`
          );
          return {
            ...oldData,
            ...updatedMovie,
