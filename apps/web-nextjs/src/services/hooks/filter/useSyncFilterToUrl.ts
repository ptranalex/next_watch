...
// Define which URL parameters are filter-related
...
export function useSyncFilterToUrl() {
  ...
  useEffect(() => {
    ...
    logger.debug(`Starting with existing params: ${params.toString()}`);

    // Remove existing filter parameters first
    FILTER_PARAMS.forEach((filterParam) => {
      if (params.has(filterParam)) {
        params.delete(filterParam);
        logger.debug(`Removed existing filter param: ${filterParam}`);
      }
    });
    ...
    Object.entries(filters).forEach(([key, value]) => {
      ...
      if (isFilterLocked(key as keyof typeof filters)) {
        logger.debug(`Skipping locked filter: ${key}`);
        return;
      }

      // Only set filter parameters that are in our FILTER_PARAMS set
      if (FILTER_PARAMS.has(key)) {
        // Skip empty values
        if (value !== undefined && value !== null && value !== "") {
          params.set(key, String(value));
          logger.debug(`Setting filter param: ${key}=${value}`);
        }
      }
    });
