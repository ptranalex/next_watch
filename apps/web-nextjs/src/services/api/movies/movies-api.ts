...
const logger = createLogger("MoviesAPI");

/**
 * Movies API - All movie-related operations through the BFF layer
 * This provides better performance, caching, and user experience optimization
 *
 * This is the updated version using the core API client instead of the BFF client
 */
export const MoviesAPI = {
  ...
  getHomeScreen: async (userId?: number): Promise<HomeScreenData> => {
    ...
    return fetchData<HomeScreenData>(endpoint);
  },
  ...
  getMoviesByGenre: async (
    ...
    return fetchData<GenreScreenData>(endpoint);
  },
  ...
  searchMovies: async (
    ...
    return fetchData<SearchResults>(endpoint);
  },
