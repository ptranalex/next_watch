...
function log(
  ...
  if (
    ...
    config.disabledGroups.has(group)
  ) {
    return;
  }
...
export function createLogger(group?: string) {
  ...
  return {
    ...
    debugOnce: (message: string, ...args: any[]) =>
      logOnce(LogLevel.DEBUG, loggerGroup, message, ...args),

    infoOnce: (message: string, ...args: any[]) =>
    ...
    errorOnce: (message: string, ...args: any[]) =>
