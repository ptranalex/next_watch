...
def find_project_root(start_path: Optional[Path] = None) -> Path:
    ...
    # Fallback: assume project root is 4 levels up from this file
    # (src/data_importer/config/env.py -> project_root)
...
def load_environment_variables(project_root: Optional[Path] = None) -> bool:
    ...
    Example:
        ...
        True
        >>> os.getenv("TMDB_ACCESS_TOKEN")  # Now available if set in .env files
        'your_token_here'
    """
    try:
...
def get_env_var(key: str, default: Optional[str] = None, required: bool = False) -> Optional[str]:
    ...
    Example:
        >>> get_env_var("TMDB_ACCESS_TOKEN", required=True)
        'your_token_here'
...
def get_env_int(key: str, default: Optional[int] = None) -> Optional[int]:
    ...
    Example:
        ...
        >>> get_env_int("PORT")
        8080
        >>> get_env_int("MISSING_VAR", default=3000)
        3000
    """
    value = os.getenv(key)
    if not value:
