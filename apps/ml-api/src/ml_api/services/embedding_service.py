...
# Type hint for SentenceTransformer
# Define a type alias that works at runtime and for static type checking
if TYPE_CHECKING:
    # Only used during type checking
    from sentence_transformers import SentenceTransformer as STModel
else:
    # Runtime behavior - try to import or use Any as fallback
    try:
        from sentence_transformers import SentenceTransformer as STModel
    except ImportError:
        STModel = Any
...
class EmbeddingService:
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            # Pass model_kwargs as keyword arguments to avoid type errors
    ...
    def generate_movie_embedding(
        ...
        if not self._model and not self.load_model():
            # If model loading failed, return a zero vector
            logger.warning(
    ...
    def generate_user_preference_vector(
        ...
        # 1. Generate embeddings for each liked movie
        # 2. Weight them by the user's rating
        # 3. Combine them with genre preferences
        # 4. Normalize the resulting vector

        # For this example, we'll just create a simple representation
        # based on the genres the user watches
        ...
        if genres_text:
            ...
            # If no genre preferences, return a zero vector
    ...
    def _generate_mock_embedding(self, movie_id: str) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: Dictionary with movie_id, embedding, model_id, and dimensions.
        """
        # Generate a zero vector with the expected dimensions
        dimensions = int(self._model_info["dimensions"])
