...
def setup_logging(verbose: bool = False, quiet: bool = False):
    ...
    ***REMOVED*** Set to ERROR level in quiet mode
    if quiet:
        for logger_name in ["httpx", "qdrant_client", 
    ...
    ***REMOVED*** Check if the embedding exists
    try:
        ***REMOVED*** Try direct point retrieval
        ...
        if response:
            console.print(f"[green]✓ Movie ID {movie_id} has an embedding in the vector database[/green]")
            ...
            ***REMOVED*** Show vector status
            vector_status = "Present" if hasattr(response, 'vector') and response.vector else "Missing"
            ...
            ***REMOVED*** Show vector data if verbose and vector exists
            if verbose and hasattr(response, 'vector') and response.vector:
                ...
                ***REMOVED*** Use a safer approach to get the first elements of the vector
                if isinstance(response.vector, list):
                    ...
                    console.print("[yellow]Vector data not available in the expected format[/yellow]")
            ...
            console.print(f"[red]✗ Movie ID {movie_id} does not have an embedding in the vector database[/red]")
            ...
            ***REMOVED*** Try searching by metadata as fallback
            ...
            if similar_response and len(similar_response) > 0:
                console.print(f"[green]Found movie via metadata search (ID: {similar_response[0].id})[/green]")
                console.print("[yellow]Note: This movie exists in the vector database but may not have a vector[/yellow]")
            else:
                ...
                ***REMOVED*** Check if movie exists in database
                with get_db_context() as session:
                    ...
                    if movie:
                        ...
                        console.print("[yellow]Run 'rec-api debug recreate_embedding {movie_id}' to create the embedding[/yellow]")
                    else:
...
def similar_movies(
    ...
    min_score: float = typer.Option(0.01, "--min-score", "-s", help="Minimum similarity score (0-1)"),
    direct_search: bool = typer.Option(False, "--direct", "-d", help="Use direct repository search"),
    show_ids_only: bool = typer.Option(False, "--ids-only", "-i", help="Show only movie IDs without details"),
    ...
    try:
        ...
        ***REMOVED*** If no results with standard approach, try fallback
        if not similar_movies:
            console.print("[yellow]No results from standard search, trying fallback approach[/yellow]")
        ...
        ***REMOVED*** Display results
        if similar_movies and len(similar_movies) > 0:
            ...
            console.print(f"[yellow]No similar movies found with minimum score {min_score}[/yellow]")
            console.print("[yellow]Try lowering the minimum score with --min-score parameter[/yellow]")
        ...
        ***REMOVED*** Get collection info
        ...
        if collection_info:
            ...
            ***REMOVED*** Provide some helpful context about the data
            if hasattr(collection_info, "points_count") and hasattr(collection_info, "vectors_count"):
                if collection_info.points_count > collection_info.vectors_count:
                    ...
                    console.print(f"[yellow]⚠ Warning: {diff} points have metadata but no vector data[/yellow]")
                    console.print("[yellow]Use 'rec-api debug recreate_embedding' or 'rec-api embeddings repair_embeddings' to fix[/yellow]")
        else:
...
def recreate_embedding(
    ...
    force: bool = typer.Option(False, "--force", "-f", help="Force recreation even if embedding exists"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed progress"),
    quiet: bool = typer.Option(False, "--quiet", "-q", help="Suppress most log output"),
) -> None:
    ...
    ***REMOVED*** First check if point exists
    ...
    if point:
        ...
        if has_vector and not force:
            console.print(f"[green]Movie ID {movie_id} already has a valid vector. Use --force to recreate anyway.[/green]")
            return
        ...
        ***REMOVED*** Delete the existing point
        ...
        if success:
            console.print(f"[green]Successfully deleted existing point for movie ID {movie_id}[/green]")
        else:
    ...
    ***REMOVED*** Generate and store new embedding
    ...
    with get_db_context() as session:
        ...
        ***REMOVED*** Generate and store embedding
        ...
        if embedding:
            console.print(f"[green]✓ Successfully generated and stored embedding for movie ID {movie_id}[/green]")
            ...
            ***REMOVED*** Verify the embedding was stored correctly
            ...
            if point and point.vector is not None:
                console.print(f"[green]✓ Verification successful. Embedding stored correctly with {len(point.vector)} dimensions.[/green]")
            else:
                console.print(f"[red]✗ Verification failed. Embedding was generated but not stored correctly.[/red]")
        else:
