...
def find_project_root(start_path: Optional[Path] = None) -> Path:
    ...
    ***REMOVED*** recommendation-api/src/recommendation_api/config/env.py -> recommendation-api/
...
def load_environment_variables(project_root: Optional[Path] = None) -> bool:
    ...
    Example:
        ...
        True
        >>> os.getenv("REDIS_URL")  ***REMOVED*** Now available if set in .env files
...
def get_env_var(key: str, default: Optional[str] = None, required: bool = False) -> Optional[str]:
    ...
    Example:
        >>> get_env_var("REDIS_URL", required=True)
        'redis://localhost:6379'
