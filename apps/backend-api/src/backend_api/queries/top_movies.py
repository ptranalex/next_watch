...
def get_top_rated_movies(
    ...
    """
    # Default to current year if not all_time and no year specified
    if not all_time and year is None:
        from datetime import datetime

        year = datetime.now().year

    # Use raw SQL to avoid ORM complexities and typing issues
    query = """
    ...
    # Apply year filter if not getting all-time movies
    if not all_time and year is not None:
        query += " AND EXTRACT(YEAR FROM m.release_date) = :year"
        params["year"] = int(year)

    # For all-time filter, apply minimum votes threshold
    if all_time:
    ...
    # Add ordering - prioritize IMDB rating, then vote count as tiebreaker
    ...
    movies = list(result.all())  # Convert to List
