...
class SuggestionEngine:
    ...
    async def get_entity_suggestions(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        async with redis.asyncio.Redis(connection_pool=self._pool) as redis_client:
            ...
            try:
                ...
                # Cache read failures should not break suggestions
                pass
            ...
            # Smart substring matching: always try for 3+ char queries to enhance results
            if len(query_prefix) >= 3 and len(suggestions) < limit:
                # Calculate how many more results we need
                remaining_slots = limit - len(suggestions)

                # For insufficient results, be more aggressive with substring matching
                if len(suggestions) < min(3, limit):
                    substring_limit = min(8, max(3, remaining_slots + 3))
                else:
                    # For good results, just add a few substring matches for enhancement
                ...
                # Add substring matches that aren't already included
            ...
            # Convert suggestions to detailed entity objects using batch hydrations
            ...
            # If hydration produced fewer than requested, try to top-up using substring matches
    ...
    async def get_ranked_suggestions(
        ...
        # Start with entity suggestions
        suggestions = await self.get_entity_suggestions(query, limit)

        # If we don't have enough results and fuzzy matching is enabled
