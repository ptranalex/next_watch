...
class RecommendationClient(BaseBackendClient):
    ...
    async def _make_request(
        ...
        try:
            ...
            # Convert 404 errors to ResourceNotFoundException for better handling
            if "404" in str(e):
                # Extract movie ID from path if possible
                movie_id = "unknown"
                if "/movies/" in path:
                    try:
                        # Extract movie ID from path like "/reco/v1/movies/3577/similar"
            ...
            # Re-raise other permanent errors
            raise
    ...
    async def get_similar_movies(
        ...
        # Use inherited HTTP client methods from BaseBackendClient
        # The @optional_service_handler decorator will automatically handle errors
        # and return an empty list if the service is unavailable
        ...
        # Extract just the recommendation movie objects from the response
