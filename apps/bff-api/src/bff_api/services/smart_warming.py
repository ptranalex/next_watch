...
class BackendConnectionManager:
    ...
    def __init__(self, max_connections: int = 3, request_timeout: int = 5):
        ...
        self._circuit_breaker_timeout = 60  # seconds before trying again
...
def get_backend_connection_manager() -> BackendConnectionManager:
    ...
    if _backend_connection_manager is None:
        ...
        _backend_connection_manager = BackendConnectionManager(
            max_connections=max_connections,  # Configurable via WARMING_MAX_CONNECTIONS
            request_timeout=request_timeout,  # Configurable via WARMING_REQUEST_TIMEOUT
        )
...
class VersionAwareWarming:
    ...
    def __init__(self) -> None:
        ...
        self._version_cache: dict[str, str] = {}  # Cache of known versions
        self._warming_in_progress: dict[str, asyncio.Event] = {}  # Prevent duplicate warming
    ...
    async def check_version_needs_warming(
        ...
        if not current_version:
            return True  # No version info, assume warming needed
    ...
    async def warm_with_version_check(
        ...
        try:
            # Check if warming is already in progress for this resource
            async with self._warming_lock:
                if warming_key in self._warming_in_progress:
                    ...
                    # Wait for the ongoing warming to complete
            ...
            try:
                # Get current version if version_getter provided
                ...
                # Check if warming needed (unless forced)
                ...
                # Mark warming as complete and remove from progress tracking
                warming_event.set()
                async with self._warming_lock:
...
class BFFSmartWarming:
    ...
    def __init__(self) -> None:
        ...
        self._warming_throttle: dict[str, datetime] = {}  # Throttle warming requests
        self._throttle_window = timedelta(seconds=30)  # Minimum time between warmings

        # Log warming configuration for debugging
        settings = get_bff_settings()  # Automatically loads .env and .env.local
    ...
    async def warm_movie_interaction(
        ...
        **context: Any,
        ...
        """
        # Apply throttling to prevent overwhelming backend
        ...
        async def _warm_movie_content() -> None:
            ...
            try:
                ...
                async def warming_func() -> None:
                    ...
                    # Reduce cascade warming - only warm genre if explicitly requested
        ...
        **context: Any,
        ...
        async def _version_aware_movie_warming() -> None:
            ...
            async def get_movie_version(movie_id: int) -> str | None:
                ...
                try:
                    # Use shared connection manager to prevent overload
                    async with self.connection_manager.get_connection() as backend_client:
                        # Get basic movie data to extract version
                        movie_data = await backend_client.get_movie(movie_id)

                        # Import version extraction function
    ...
    async def warm_priority_movies(
        ...
        background_tasks: BackgroundTasks,
        priority_tier: int = 1,
        max_movies: int | None = None,  # Now optional for unlimited warming by default
        force: bool = False,
        **context: Any,
    ) -> None:
        ...
        async def _priority_warming() -> None:
            ...
            try:
                # Safety warning for very high limits
                ...
                # Get movie IDs based on tier and max_movies parameter
                ...
                # Use controlled concurrency to balance speed and stability
                max_concurrent = min(
                    4, len(movie_ids)
                )  # Increased from 2 to 4 for better performance
                ...
                # Warm movies with version checking to avoid redundant work
                ...
                # Execute warming tasks with bounded concurrency
    ...
    async def _get_tier_movie_ids(self, priority_tier: int, max_movies: int | None) -> list[int]:
        ...
        """
        try:
            # Determine if we should use debugging limit or get all movies
            use_debug_limit = (
                max_movies is not None and max_movies < 10000
            )  # Assume limits < 10k are for debugging

            if priority_tier == 1:
                # Tier 1: New releases + trending (last 30 days)
                if use_debug_limit:
                    logger.info(
                    ...
                    return await self._get_new_and_trending_movies(None)  # None = no limit

            elif priority_tier == 2:
                # Tier 2: Popular movies + user favorites
                if use_debug_limit:
                    logger.info(
                    ...
                    return await self._get_popular_movies(None)  # None = no limit

            elif priority_tier == 3:
                # Tier 3: Full catalog for discovery
                if use_debug_limit:
                    logger.info(
                    ...
                    return discovery_ids[:max_movies]  # Apply debug limit
                else:
    ...
    async def _get_new_and_trending_movies(self, max_movies: int | None) -> list[int]:
        ...
        try:
            # Get actual movie IDs from backend instead of fake ranges
            async with self.connection_manager.get_connection() as backend_client:
                # Get movies - use unlimited fetch if no max_movies specified
                if max_movies is None:
                    # No limit - get all available movies and return the recent ones
                    all_movies = await self._get_all_available_movies()
                    logger.info(f"Retrieved {len(all_movies)} total movies for Tier 1 filtering")
                    return (
                        all_movies  # For Tier 1, return all (could add date filtering logic here)
                    )
                else:
                    # Limited fetch for debugging
                    ...
                    try:
                        ...
                        # Fallback to basic call without sorting
            ...
            # Fallback: Get a smaller sample of existing movies
    ...
    async def _get_popular_movies(self, max_movies: int | None) -> list[int]:
        ...
        try:
            # Get actual popular movies from backend
            async with self.connection_manager.get_connection() as backend_client:
                # Get movies - use unlimited fetch if no max_movies specified
                if max_movies is None:
                    # No limit - get all available movies
                    all_movies = await self._get_all_available_movies()
            ...
            # Fallback: Get a sample of existing movies
    ...
    async def _get_discovery_movies(self, max_movies: int) -> list[int]:
        ...
        try:
            # Get the full movie catalog from backend for discovery
            async with self.connection_manager.get_connection() as backend_client:
                # For discovery, get a diverse set - could be random or by various criteria
                # We'll paginate through movies to get up to max_movies
                all_movie_ids: list[int] = []
                page = 1
                page_size = min(100, max_movies)  # Backend API limit is 100 per request

                while len(all_movie_ids) < max_movies:
                    remaining = max_movies - len(all_movie_ids)
                    current_limit = min(page_size, remaining)

                    # Try basic get_movies call (without sorting to avoid 400 errors)
                    try:
                    ...
                    if not page_movie_ids:  # No more movies
                        logger.warning(
                    ...
                    # Backend reports 179 total pages, so allow up to 200 for safety
                    max_pages = max(
                        (max_movies // page_size) + 10, 200
                    )  # Allow enough pages + buffer
                    if page > max_pages:
                        logger.warning(
            ...
            # Fallback: Get whatever movies we can find
    ...
    async def _get_bulk_movie_ids(self, max_movies: int) -> list[int]:
        ...
        try:
            async with self.connection_manager.get_connection() as backend_client:
                # Try using bulk endpoint with sequential ID ranges
                all_movie_ids: list[int] = []

                # Start with a reasonable ID range (most movie DBs start at 1)
                start_id = 1
                batch_size = 100

                while len(all_movie_ids) < max_movies and start_id < 10000:  # Safety limit
                    end_id = min(
                    ...
                    try:
                        ...
                        # If bulk fails, this isn't the right approach for this backend
                        break
    ...
    async def _get_all_available_movies(self) -> list[int]:
        ...
        try:
            async with self.connection_manager.get_connection() as backend_client:
                all_movie_ids: list[int] = []
                page = 1
                page_size = 100  # Backend API limit is 100 per request
                ...
                while True:
                    try:
                        ...
                        # Log total pages on first response
                        ...
                        # Safety break - but much higher since we want everything
                        if page > 300:  # Should handle up to 30,000 movies
            ...
            # Fallback to the old discovery method with a high limit
    ...
    async def _get_fallback_movie_ids(self, max_movies: int) -> list[int]:
        ...
        try:
            async with self.connection_manager.get_connection() as backend_client:
                # Get movies in small pages to find existing IDs
                all_movie_ids: list[int] = []
                page = 1
                page_size = 50  # Small pages for safety
                ...
                while len(all_movie_ids) < max_movies:
                    ...
                    try:
                        # Try simplest possible API call to avoid parameter issues
                        if page == 1:
                            # First try - use limit parameter
                            movies_response = await backend_client.get_movies(
                                page=1, limit=current_limit
                            )
                        else:
                            # For pagination, some backends may not support it
                            movies_response = await backend_client.get_movies(
                                page=page, limit=current_limit
                            )
                        ...
                        # Safety break (increased for large datasets)
            ...
            # Last resort: try the absolute simplest API call
            try:
                async with self.connection_manager.get_connection() as backend_client:
                    # Most basic call possible - no parameters
                    movies_response = await backend_client.get_movies()
                    if movies_response and "results" in movies_response:
                        movie_ids = []
            ...
            # Ultimate fallback: small safe range (only if all else fails)
    ...
    async def _warm_single_movie_with_version(
        ...
        async def get_movie_version(movie_id: int) -> str | None:
            ...
            try:
                # Use shared connection manager for version checks
                async with self.connection_manager.get_connection() as backend_client:
                    movie_data = await backend_client.get_movie(movie_id)
        ...
        **context: Any,
        ...
        async def _warm_search_content() -> None:
            ...
            async def warming_func() -> None:
                ...
                # TODO: Implement actual search warming logic
    ...
    async def warm_user_registration(
        ...
        async def _warm_new_user_content() -> None:
            ...
            async def warming_func() -> None:
                ...
                # TODO: Implement actual new user warming logic
        ...
        **context: Any,
        ...
        async def _warm_missed_content() -> None:
            ...
            async def warming_func() -> None:
                ...
                # TODO: Implement actual cache miss warming logic
                logger.debug(
